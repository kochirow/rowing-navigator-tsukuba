...
/// 艇そのものの表示色を一元管理する。
///
/// 危険区域の色は [HazardPalette]、航路中央線の色は `channelDividerStyleFor`
/// が持つ。
...
class BoatPalette {
  ...
  /// 航行中の自艇の色。**白**(2026-09-26 利用者決定)。
  ///
  /// 以前は赤で「赤は自艇だけ」としていたが、警告を画面の端の赤い点滅で
  /// 出すようにしたため、赤は「衝突に関わるもの(他艇・警告)」に使う。
  /// 自艇は航行中は画面の定位置(横中央・上から45%)にいるので、色で探す
  /// 必要がない。暗い地図の上で一番明るい記号にする(航空機の衝突防止表示
  /// TCAS でも自機は白)。縁は明るさから自動で黒になる(`image2icon.dart`)。
  static const Color myBoat = Color(0xFFFFFFFF);

  /// 航行モードの他艇の色。**常に赤系**(2026-09-26 利用者決定)。
  ///
  /// 他艇の位置は、離れていても常に把握しておきたい(万一の事故のとき
  /// 素早く動ける)。暗い地図の上で最も目立つ赤系にする。自艇(白)とは
  /// 被らない。警告の相手は色ではなく強調(縁・点滅)で区別する。
  /// 明るさは 0.5 未満なので、艇印の縁は白になる。
  ///
  /// **航行モードでは他艇を艇ごとに色分けしない。** 12色を暗記させないと
  /// 意味を持たない情報である。誰かを知りたいときのために名前ラベルを
  /// 常時出してある。
  static const Color otherBoat = Color(0xFFFF5A5A);
