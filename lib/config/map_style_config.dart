...
///
/// 漕手は後ろ向きに座り進行方向が見えない。地図は進行方位+180度で回して
/// いる(`rowingMapBearing`)ので、**進行方向は画面の下**になり、自艇の印も
/// 先端が下を向く。自艇より下が前方、上が後方である。
///
/// **0.45(やや上寄り)。** 画面上部は計器カードが覆うので、自艇を上寄りに
/// 置けば、カードに隠れない下側の55%を前方の見通しに使える。以前の 0.667
/// (下から1/3)では前方が画面の1/3しかなかった。
/// (2026-08-03 にこの値にしたときの説明は「進行方向が上」と書いていたが、
/// 実装は当時から+180度だった。2026-09-25 に説明を実装へ合わせた。)
/// 前方の見通しを増やしたいときは、この値を下げる(自艇を上へ寄せる)か、
/// 計器カードを小さくすること。
///
